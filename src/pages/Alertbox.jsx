...
const Alertbox = () => {
  ...
  return (
    <div className="App">
      <div className="layer text-start" id="log" style={{"display": (layer.current === "log" ? "block" : "none")}}>
        {log.map((text, i) => (<p key={i} className="mb-0">{text}</p>))}
      </div>
      <div className="layer" id="play" style={{"display":(layer.current === "play" ? "block" : "none")}}></div>
      <div className="layer" id="setting" style={{"display":(layer.current === "setting" ? "block" : "none")}}>
