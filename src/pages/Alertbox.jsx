...
const Alertbox = () => {
  ...
  useEffect(() => {
    ...
    if(id){
      ...
      changeLog(prevLog => [...prevLog, 'URL invalid']);
    }
  }, []);
  ...
  return (
    <div className="App">
      <div className="layer text-start" id="log" style={{"display": (layout.current === "log" ? "block" : "none")}}>
        {log.map((text, i) => (<p key={i} className="mb-0">{text}</p>))}
      </div>
      <div className="layer" id="play" style={{"display":(layout.current === "play" ? "block" : "none")}}></div>
      <div className="layer" id="setting" style={{"display":(layout.current === "setting" ? "block" : "none")}}>
